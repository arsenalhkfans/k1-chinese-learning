...
function findVoiceByLanguage(voices, preferredLangs, excludedLangPrefixes = []) {
  const filteredVoices = voices.filter((voice) => !isExcludedVoice(voice.lang, excludedLangPrefixes));
...
async function speakCurrentChar(mode) {
  ...
  const selectedVoice = findVoiceByLanguage(voices, currentConfig.preferredLangs, currentConfig.excludedLangPrefixes);
